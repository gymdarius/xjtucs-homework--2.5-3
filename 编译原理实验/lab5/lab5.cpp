                ...
                if (tt.left[0] == 'S')
                    ...
                    for (int k = 0; k < V.size(); k++)
                    {
                        ...
                        if (!follow[tt.left].count(V[k]))
                            continue;
                        // 判断是否是slr(1)文法,如果在相同位置已经填过数据，则发生了冲突，不是slr(1)文法
                        if (action[i][y].type != -1)
                        {
                            cout << "状态" << i << " " << "发生冲突的符号" << (char)y << endl;
                            cout << "不是slr(1)文法" << endl;
                        }
...
{
    std::ofstream outfile("4yuan.txt");
